...
function logNotification(message: string, clear: boolean = false) {
...
function showInfo(info: HTMLElement) {
...
class WordContentCtrls {
    ...
    protected readonly OPTIONS = [this.RTSelectTag, 'RTShow', 'RTEdit'];

    /**
     * 
     * @param ctrls 
     */
    protected async insertFields(ctrls: ContentControl[]) {
        for (const ctrl of ctrls) {
            try {
                const start = ctrl.getRange(Word.RangeLocation.before);
                const field = await this.insertSingleFiled(start, ctrls.indexOf(ctrl));
                if (field) field.untrack();
                ctrl.untrack();
    ...
    protected async insertSingleFiled(range?: Word.Range | void, i: number = 0, style: string = '') {
        if (!range) {
     ...
     */
    protected async insertContentControl(range: Word.Range | Word.Paragraph, title: string, tag: string, index: number = 1, type: ContentControlType, style: string | undefined, cannotEdit: boolean = true, cannotDelete: boolean = true, placeHolder?: string, props: string[] = []): Promise<ContentControl | undefined> {
    ...
    protected async wrapSelectionWithContentControl(title: string, tag: string, type: ContentControlType, cannotEdit: boolean, cannotDelete: boolean, style?: string) {
        const range = await this.getSelectionRange();
        if (!range) return;
        if (!style && this.RTSiStyles.includes(range.style)) style = range.style || undefined;
    ...
    protected getFirstByTag(range: Word.Range | ContentControl, tag: string) {
        return range.getContentControls().getByTag(tag).getFirst();
    }

    protected getCtrlTitle(tag: string, id: number) {
        return `${tag}&${id}`
    }
    ...
    protected async setFieldTitle(title: string, field?: ContentControl) {
        await Word.run(async (context) => {
            ...
            await context.sync();
            if (field?.tag !== this.RTFieldTag) return console.log('field tag !== RTFieldTag')
    ...
    protected async updateAllFields(field: ContentControl | undefined) {
        await Word.run(async (context) => {
...
export class EditContract extends WordContentCtrls {
    ...
    private prepareTemplate() {
        ...
        const searchString = this.searchString.bind(this),
            ...
            insertFILLINField = this._fields.insertFIllINField.bind(this._fields);
        ...
        function btn(title: string, tag: string, type: ContentControlType, cannotEdit: boolean, cannotDelete: boolean, label: string, style?: string, hint?: string) {
            return {
        ...
        async function insertRTDescription(selection: boolean = false, style: string = `${stylePrefix}Normal`) {
            ...
            else ctrls = await findTextAndWrapItWithContentControl([descStyle], descTag, descTag, false, true);
        ...
        async function automaticallyInsertSelectWrapers(): Promise<void> {
            ...
            await Word.run(async (context) => {
                ...
                if (!main?.id) return logNotification('The select wraper id was not retrieved');
                try {
                ...
                async function processSelectWraper(select: ContentControl) {
                    if (!select) return;

                    if (!select.paragraphs.items.length) return logNotification('There are no paragraphs within the RTSelect wraper range');
                    ...
                    async function process(siParags: Word.Paragraph[]) {
                        if (!siParags.length) return;
        ...
        async function insertBlockAmount() {
            ...
            function getField(code: string, ctrl: ContentControl) {
        ...
        async function findTextAndWrapItWithContentControl(styles: string[], title: string, tag: string, cannotEdit: boolean, cannotDelete: boolean) {
            ...
            if (!styles?.length) return logNotification(`The styles[] has 0 length, no styles are included, the function will return`);
            if (!search?.length) return logNotification('The provided search string is not valid');
    ...
    private async customizeContract(showNested: boolean = false) {
        ...
        async function loopSelectCtrls() {
            await Word.run(async (context) => {
                if (showNested) return await showNestedOptionsTree(context);//!This must come before selectCtrls is populated. Will populate it from the selection
                ...
                try {
                    ...
                    if (!await promptConfirm('Do you want to delete the unselected contentcontrols?\nWARNING: If you click NO, all the excluded options will be lost and you will have to start over again')) return;
        ...
        async function labelRange(id: number, context: Word.RequestContext) {
            ...
            label.load(['cannotEdit', 'font/hidden']);
            await context.sync();
        ...
        async function promptForSelection(ctrls: selectCtrl[], context: Word.RequestContext, clear: boolean = true) {
            ...
            async function processCtrls() {
                ...
                async function processCtrl(ctrl: selectCtrl) {
                    ...
                    if (ctrl.tag === RTSelect) await processSelect(ctrl);
                    else if (ctrl.tag === RTClone) await processClone(ctrl);
                }

                async function processSelect(ctrl: selectCtrl) {
                ...
                async function processClone(ctrl: selectCtrl) {
                ...
                async function showPromptUI(ctrl: selectCtrl) {
                    ...
                    if (!block) return;
                    blocks.push(block);
                    if (block.btnNext) await btnOnClick(block.btnNext);//This is the case where btnNext was added because we reached the end of ctrls[] (isLast = true). We then need to await the user to click the button in order to process all the already displayed html elements/options of ctrls[].

                    async function insertHtml(ctrl: selectCtrl): Promise<promptBlock | void> {
                        ...
                        if (ctrl !== ctrls[ctrls.length - 1]) return { wraper, checkBox, ctrl };
                    ...
                    function next(ctrl: selectCtrl, wraper: HTMLDivElement) {
                        ...
                        const btnNext = element<HTMLButtonElement>('button', undefined, 'Next', btns);
                        back(ctrl, btns);
                        return btnNext
                    }
                    ...
                    function btnOnClick(btn: HTMLButtonElement): Promise<boolean> {
                ...
                async function isSelected(ctrl: selectCtrl, selected: boolean) {
                    if (selected)
                ...
                function insertWraper(parent: HTMLElement) {
                    return element<HTMLDivElement>('div', 'promptContainer', '', parent);
                }

                async function insertLabel(id: number, wraper?: HTMLElement) {
                    if (!wraper) wraper = insertWraper(USERFORM);
            ...
            async function cloneSelectBlock(ctrl: selectCtrl) {
                ...
                async function processClone(id: number, text: string, i: number) {
                    ...
                    if (!clone) return showAlert('The clone could not be retrived from the selectCtrls array !');
                    ...
                    if (clone.hasLabel?.tag !== RTSectionTag) return showAlert('The clone does not have an RTSection label !');//Normally this should never occur.

                    const label = await labelRange(clone.hasLabel.id, context);
                    if (!label) throw new Error('Failed to retrive the label of the Clone');//This should never occur.
                    text = `${text}-${i}`;
        ...
        async function fetchSelectCtrls(context: Word.RequestContext, allRT?: Word.ContentControlCollection) {
            ...
            function selectCtrl(ctrl: ContentControl): selectCtrl {
                return {
                    id: ctrl.id,
                    tag: ctrl.tag,
                    title: ctrl.title,
            ...
            function hasLabel(ctrl: ContentControl) {
        ...
        async function showNestedOptionsTree(context: Word.RequestContext) {
            ...
            if (!ctrls.items.length) return showAlert('You must select a range containing the RTSelect ContentContrls you want to show its tree');//!This MUST COME AFTER selectCtrls.push() because ctrls.items are not available until fetchSelectCtrls() loads their properties.
    ...
    private async editExistingFields() {
        ...
        await Word.run(async (context) => {
            ...
            async function getInput(field: Word.Field) {
                ...
                async function onChange(question: string) {
            ...
            async function awaitPromise(inputs: [HTMLInputElement, Word.Field][]) {
                return new Promise((resolve) => {
                    const edit = async (cancel: boolean = false) => {
                        if (cancel) return resolve(undefined);
